...
def BesTo_msg(mess, data, clin):
    ...
    try:
        ...
        return f"An error occurred: {e}"    
def likes_plus(uid):
    url = f'https://ff-community-api.vercel.app/sendLikes?uid={uid}&access_key=55sub'
    response = requests.get(url)
                    ...
                    if '0e00' in dataS.hex()[:4] and '0e15' in dataC.hex()[:4] and preventlag == True:
                        ...
                        if '0515' in dataC.hex()[0:4] and len(dataC.hex()) >= 900 and spam_invs == True :
                                try:
                                    ...
                                    remote.close()
                                    client.close()                
                        if "0f0000" in dataS.hex()[0:6] and "0f15" in dataC.hex()[0:4] and mess ==True:
                            time.sleep(1)
                            id_add = dataS.hex()[40:50]
                            try:
                                clientC.send(bytes.fromhex(f"060000006808d4d7faba1d100620022a5c08{id_add}1a1b5b3030464630305d4d5224e385a46672656e645b3030464646465d32024d45404db00113b801a528d801d4d8d0ad03e00101b801e807f00101f8019a018002fd98a8dd03900201d0020cd8022ee002b2e9f7b103"))
                            except:
                                pass
                        if "0f0000" in dataS.hex()[0:6] and len(dataS.hex()) == 52 and "0f15" in dataC.hex()[0:4] and mess ==True:
                            time.sleep(1)
                            id_add = dataS.hex()[-10:]
                            clientC.send(bytes.fromhex(f"060000006808d4d7faba1d100620022a5c08{id_add}1a1b5b3030464630305d4d5224e385a46672656e645b3030464646465d32024d45404db00113b801a528d801d4d8d0ad03e00101b801e807f00101f8019a018002fd98a8dd03900201d0020cd8022ee002b2e9f7b103"))
...
##################################
                        ...
                        if b"/pc" in dataS and comand == True:
                            ...
                            BesTo_msg(f"""[cُ][bَ][F87431]- تم الحصول على شعار البيسي\n\n- By : Rbgx Team""", dataS.hex(), client)
 ...
 #################################
                        ...
                        if b"/5s" in dataS and comand == True:
                            BesTo_msg(f"""[cُ][bَ][00FFFF]- تم تفعيل سكواد خمسة\n\n- By : Rbgx Team""", dataS.hex(), client)
                            id = dataS.hex()[12:22]
...
##################################
                        ...
                        if b'/fr' in dataS and comand ==True:    
                            mess = True                            
                            x = 0
                            if len(dataS.hex())<=30:
                                hide = True
                            if len(dataS.hex())>=25:
                                ...
                                liner = client                                
                                spyN = dataS
                                BesTo_msg(f"""[b][c][00FFFF]- تم تمفعيل الصديق الوهمي\n\n- By : Rbgx Team""", dataS.hex(), client)              
...
##################################
                        ...
                        if b"/inv" in dataS and comand == True:
                            ...
                            BesTo_msg(f"""[cُ][bَ][FFFF00]- تم تفعيل سبام دعوات\n\n- By : Rbgx Team""", dataS.hex(), client)
...
#################################                              
                        ...
                        if b'/gold' in dataS and comand ==True:
                            ...
                            BesTo_msg(f"""[cُ][bَ][00CED1]- تم اضافة 55,000 من الغلود\n\n
 """, dataS.hex(), client)
...
##################################
                        ...
                        if b'/dis' in dataS and comand ==True:
                            BesTo_msg(f"""[b][c][ffe75c]- تم تصفير الجواهر""", dataS.hex(), client)
...
#################################
 ...
 ################################
                        ...
                        if b"/yout" in dataS:
                            ...
                            BesTo_msg(f"""[cُ][bَ][00FF00]- تم اضافة يوتيوبر للاصدقاء\n\n- By : Rbgx Team""", dataS.hex(), client)
...
##################################
                        ...
                        if b"/spy" in dataS:
                            ...
                            BesTo_msg(f"""[cُ][bَ][40E0D0]- تم اختفاء الاعب ابقا صامت !\n\n- By : Rbgx Team""", dataS.hex(), client)
